...
class Debugger(Tracer):
    ...
    def condbreak_command(self, arg: str) -> None:
        """Break when arg given as argument is True. If no arg given - print all conditions."""
        if arg:
            self.break_conditions.add(arg)
        self.log("Break conditions:", self.break_conditions)

    def delcond_command(self, arg: str) -> None:
        """Delete given break condition. Without argument - clear all conditions."""
        if arg:
            try:
                self.break_conditions.remove(arg)
            except KeyError:
                self.log(f"No such break condition: {arg}")
        else:
            self.break_conditions = set()
        self.log("Break conditions:", self.break_conditions)
