...
class Debugger(Tracer):
    ...
    def __init__(self, *, file: TextIO = sys.stdout) -> None:
        ...
        # Line : condition
        self.breakpoints: dict[int, str] = dict()
    ...
    def stop_here(self) -> bool:
        ...
        if self.frame.f_lineno in self.breakpoints.keys():
            should_break = bool(eval(self.breakpoints[self.frame.f_lineno], self.frame.f_globals, self.local_vars))
        else:
            should_break = False
        return self.stepping or should_break
    ...
    def break_command(self, args: str = "") -> None:
        """Set a breakoint in given line. If no line is given, list all breakpoints"""

        if args:
            args_list = args.split(" ")
            break_line = args_list[0]
            break_cond = args_list[1] if len(args_list) > 1 else "True"
            self.breakpoints[int(break_line)] = break_cond
...
if __name__ == "__main__":
    clear_next_inputs()
    next_inputs(["step", "step", "break 100", "continue", "step", "continue"])
    with Debugger() as debugged:
