...
import typing
import warnings
from ast import (AST, AnnAssign, Assert, Assign, Attribute, AugAssign, Call,
                 Expr, Load, Name, NodeTransformer, NodeVisitor, Store, With,
                 keyword, withitem)
...
class Dependencies(StackInspector):
    ...
    def __init__(
        self, data: Optional[Dependency] = None, control: Optional[Dependency] = None
    ) -> None:
        """
    ...
    def make_graph(
        self, name: str = "dependencies", comment: str = "Dependencies"
    ) -> Digraph:
        return Digraph(
            name=name,
            comment=comment,
    ...
    def draw_edge(
        self, g: Digraph, mode: str, node_from: str, node_to: str, **kwargs: Any
    ) -> None:
        if mode == "flow":
    ...
    def label(self, var: Node) -> str:
        ...
        if source:
            label += (
                f'<FONT POINT-SIZE="9.0"><BR/><BR/>' f"{html.escape(source)}" f"</FONT>"
            )
        label = f"<{label}>"
        return label
    ...
    def expand_criteria(self, criteria: List[Criterion]) -> List[Node]:
        ...
        for criterion in criteria:
            ...
            for var in self.all_vars():
                ...
                name_matches = (
                    criterion_func is None
                    or criterion_func == func
                    or criterion_func.__name__ == func.__name__
                )

                location_matches = (
                    criterion_lineno is None or criterion_lineno == lineno
                )
    ...
    def format_var(self, var: Node, current_func: Optional[Callable] = None) -> str:
        ...
        func, lineno = location
        if current_func and (
            func == current_func or func.__name__ == current_func.__name__
        ):
    ...
    def repr_dependencies(self, vars: Dependency) -> str:
        return (
            "{\n        "
            + ",\n        ".join(
                f"{self.repr_var(var)}: {self.repr_deps(vars[var])}" for var in vars
            )
            + "}"
        )
...
class DataTracker(StackInspector):
    ...
    def arg(
        self, value: Any, pos: Optional[int] = None, kw: Optional[str] = None
    ) -> Any:
        """
...
class DependencyTracker(DataTracker):
    ...
    def set(self, name: str, value: Any, loads: Optional[Set[str]] = None) -> Any:
        ...
        def add_dependencies(
            dependencies: Set[Node], vars_read: List[str], tp: str
        ) -> None:
        ...
        add_dependencies(
            self.data_dependencies.setdefault((name, location), set()),
            self.last_read,
            tp="data",
        )
        add_dependencies(
